package br.com.sicoob.sisbr.cca.cadastro.plataforma
{
	...
	public class ParcelamentoCadastro extends ParcelamentoCadastroView{		
		...
		public 	var devolucaoRenEdicao:DevolucaoRenEdicao;
		...
		private function controlarTela():void {
			...
			if(bolTelaPlataformaAtendimento){
				dtVencimento.selectedDate = DataUtilRelatorios.somarDiasData(DataUtilRelatorios.stringBrToDate(devolucaoRenEdicao.dataUltimaParcela), 30);				
			}
		...
		private function incluir():void {
			if(validar()) {
				...
				if(bolTelaPlataformaAtendimento){
					devolucaoRenEdicao.listaParcelas.addItem({dataVencimento:FormataData.formataData(dtVencimento.selectedDate, "DD/MM/YYYY"),valorParcela:vlrParcela.valor,idTipoInteg:idTipoInteg,numContaCorrente:cboCcoParcela.text,dataVencimentoOrdenacao:dataVencimentoOrdenacao});				
					devolucaoRenEdicao.atualizaValores();														
				}
		...
		private function validar():Boolean {			
			...
			if(bolTelaPlataformaAtendimento){
				var dataDevol:String = FormataData.formataData(devolucaoRenEdicao.dtInicioParcelamento.selectedDate, "YYYYMMDD");			
			}
			...
			if(bolTelaPlataformaAtendimento){			
				if(vlrParcela.valor > devolucaoRenEdicao.vlrParcelar.valor) {
					Alerta.show("Valor da parcela deve ser menor ou igual a R$ " + devolucaoRenEdicao.vlrParcelar.text, "ATENÇÃO", Alerta.ALERTA_OK, vlrParcela);
					return false;
				}			
			}
